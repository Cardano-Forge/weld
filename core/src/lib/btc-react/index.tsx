...
const walletContext = createContextFromStore<WeldBtcInstance, "wallet">(
	"wallet",
);
...
export const useBtcWallet: {
	...
	<TKey extends keyof BtcWalletProps | keyof BtcWalletApi>(
		key: TKey,
	): BtcWalletStoreState[TKey];
	<TKeys extends ReadonlyArray<keyof BtcWalletProps | keyof BtcWalletApi>>(
		...keys: [...TKeys]
	): BtcWalletStoreState<TKeys[number]>;
} = walletContext.hook;

const extensionsContext = createContextFromStore<WeldBtcInstance, "extensions">(
	"extensions",
);
const ExtensionsProvider = extensionsContext.provider;
export const useBtcExtensions = extensionsContext.hook;

export type WeldBtcProviderProps = React.PropsWithChildren<
	Partial<WeldConfig>
> & {
