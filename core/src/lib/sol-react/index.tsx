...
const walletContext = createContextFromStore<WeldSolInstance, "wallet">(
	"wallet",
);
...
export const useSolWallet: {
	...
	<TKey extends keyof SolWalletProps | keyof SolWalletApi>(
		key: TKey,
	): SolWalletStoreState[TKey];
	<TKeys extends ReadonlyArray<keyof SolWalletProps | keyof SolWalletApi>>(
		...keys: [...TKeys]
	): SolWalletStoreState<TKeys[number]>;
} = walletContext.hook;

const extensionsContext = createContextFromStore<WeldSolInstance, "extensions">(
	"extensions",
);
const ExtensionsProvider = extensionsContext.provider;
export const useSolExtensions = extensionsContext.hook;

export type WeldSolProviderProps = React.PropsWithChildren<
	Partial<SolConfig>
> & {
