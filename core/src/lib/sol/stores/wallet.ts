...
function newSolWalletState(): PartialWithDiscriminant<
	SolWalletProps,
	"isConnected"
> {
	return {
...
export type SolWalletApi = {
	connect(
		key: SolExtensionKey,
		config?: Partial<WalletConfig & ConnectSolWalletCallbacks>,
	): void;
	connectAsync: (
		key: SolExtensionKey,
...
export type SolWalletState<
	TKeys extends keyof SolWalletProps = keyof SolWalletProps,
> = PartialWithDiscriminant<SolWalletProps, "isConnected", TKeys>;

export type ConnectedSolWalletState = Extract<
	SolWalletState,
	{ isConnected: true }
>;
export type DiconnectedSolWalletState = Extract<
	SolWalletState,
	{ isConnected: false }
>;
...
export type SolWalletStore = Store<
	SolWalletStoreState,
...
>(
	(
			...
			createConnection: async (key, opts) => {
				...
				if (!extension || !api?.publicKey) {
					throw new WalletConnectionError(
						`The ${key} extension is not installed`,
					);
				}
				...
				const endpoint =
					config.connectionEndpoint ?? defaultSolConnectionEndpoint;
		...
		const sendTokens = async (
			...
			const { signature } =
				await api.signAndSendTransaction(preparedTransaction);
		...
		const send = async (opts: SolSendOpts) => {
			...
			if (isSolUnit(unit)) {
				const amountLamports =
					unit === "lamport" ? amount : solToLamport(amount);
