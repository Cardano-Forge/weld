...
const walletContext = createContextFromStore<WeldEthInstance, "wallet">(
	"wallet",
);
...
export const useEthWallet: {
	...
	<TKey extends keyof EvmWalletProps | keyof EvmWalletApi>(
		key: TKey,
	): EvmWalletStoreState[TKey];
	<TKeys extends ReadonlyArray<keyof EvmWalletProps | keyof EvmWalletApi>>(
		...keys: [...TKeys]
	): EvmWalletStoreState<TKeys[number]>;
} = walletContext.hook;

const extensionsContext = createContextFromStore<WeldEthInstance, "extensions">(
	"extensions",
);
const ExtensionsProvider = extensionsContext.provider;
export const useEthExtensions = extensionsContext.hook;

export type WeldEthProviderProps = React.PropsWithChildren<
	Partial<EvmConfig>
> & {
