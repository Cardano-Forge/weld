...
function newGetWalletExtensionsMock({
	caching = false,
} = {}): typeof getWalletExtensions {
	return async () => {
		if (caching) {
...
function newGetWalletInfoMock(
	supportedKeys: string[] = [],
): typeof getWalletInfo {
	return (extension) => {
		return {
...
describe("getInstalledExtensions", () => {
	...
	it("should classify extensions", async () => {
		...
		expect(
			res.unsupportedArr.find((e) => e.info.key === "eternl"),
		).not.toBeUndefined();
		expect(res.supportedMap.get("lace")?.info.key).toBe("lace");
		expect(
			res.supportedArr.find((e) => e.info.key === "lace"),
		).not.toBeUndefined();
	});
