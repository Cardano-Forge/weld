...
describe("connectAsync", () => {
	it("should connect to valid installed wallets successfully", async () => {
		...
		const handler = new DefaultWalletHandler(
			laceInfo,
			defaultApi,
			enabledApi,
			enable,
	...
	it("should connect over existing connections", async () => {
		...
		const { DefaultWalletHandler } = await import("@/internal/handler");
		const laceHandler = new DefaultWalletHandler(
			laceInfo,
			defaultApi,
			enabledApi,
			enable,
		);
		const eternlHandler = new DefaultWalletHandler(
			eternlInfo,
			defaultApi,
			enabledApi,
			enable,
	...
	it("should give up after maxUtxosUpdateRetryCount", async () => {
		...
		await vi.advanceTimersByTimeAsync(
			utxosUpdateRetryInterval * maxUtxosUpdateRetryCount * 2,
		);
