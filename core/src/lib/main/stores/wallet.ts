...
export type WalletApi = {
	connect(
		key: string,
		config?: Partial<WalletConfig & ConnectWalletCallbacks>,
	): void;
	connectAsync: (
		key: string,
...
export type DiconnectedWalletState = Extract<
	WalletState,
	{ isConnected: false }
>;

export type WalletStoreState<
	TKeys extends keyof WalletProps | keyof WalletApi =
		| keyof WalletProps
		| keyof WalletApi,
	...
	(
			...
			createConnection: async (key, opts) => {
				...
				const updateUtxos = ({ expectChange = false } = {}) => {
					...
					getNextUtxos({ expectChange })
						...
						.catch((error) => {
							walletManager
								.handleUpdateError(
									new WalletUtxosUpdateError(getFailureReason(error)),
								)
								.then(() => {
									if (
										!running.update.signal.aborted &&
										!handler.isDisconnected
									) {
										setState({ isUpdatingUtxos: false, utxos: [] });
									}
									running.resolve([]);
				...
				const updateState = async () => {
					...
					const hasAccountChanged =
						changeAddressHex !== prevState.changeAddressHex;
		...
		const updateCookieState = (newState: Partial<WalletState>) => {
			...
			if (newState.changeAddressHex) {
				config.storage.set(
					STORAGE_KEYS.connectedChange,
					newState.changeAddressHex,
				);
			} else {
			...
			if (newState.stakeAddressHex) {
				config.storage.set(
					STORAGE_KEYS.connectedStake,
					newState.stakeAddressHex,
				);
			} else {
		...
		const connectAsync: WalletApi["connectAsync"] = async (
			key,
			configOverrides,
		) => {
			await walletManager.disconnect();
		...
		const connect: WalletApi["connect"] = async (
			key,
		...
		const __persist = (data?: WalletStorePersistData) => {
			...
			let changeAddressHex: ChangeAddressHex | undefined;
			let stakeAddressHex: StakeAddressHex | undefined;

			const canPersistFromCookies =
				typeof window !== "undefined" && config.enablePersistence;
