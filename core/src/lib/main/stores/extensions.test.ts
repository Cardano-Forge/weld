...
describe("update", () => {
	it("should save valid extensions", async () => {
		...
		expect(store.supportedMap.get("lace")?.defaultApi).toBe(
			window.cardano.lace,
		);
		expect(
			store.unsupportedMap.get("unsupported")?.defaultApi,
		).not.toBeUndefined();
		expect(store.unsupportedMap.get("unsupported")?.defaultApi).toBe(
			window.cardano.unsupported,
		);
	});
