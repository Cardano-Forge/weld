...
import { isXverseApi, type Protocol, type XverseApi } from "./types";
...
class XverseBtcWalletHandler implements BtcWalletHandler {
	...
	async getPaymentAddress(): Promise<string> {
		await this.checkPermissions();
		const res = await this._ctx.api.request("getAddresses", {
			purposes: ["payment"],
		});
		if ("error" in res) {
	...
	async getInscriptions({
		...
		const res = await this._ctx.api.request("ord_getInscriptions", {
			limit,
			offset,
		});
		if ("error" in res) {
	...
	async signMessage(
		...
		const res = await this._ctx.api.request("signMessage", {
			address,
			message,
			protocol,
		});
	...
	async signPsbt(psbtHex: string, opts: SignPsbtOpts): Promise<SignPsbtResult> {
		...
		const res = await this._ctx.api.request("signPsbt", {
			psbt: psbtBase64,
			signInputs,
		});
		if ("error" in res) {
	...
	async checkPermissions() {
		const curr = await this._ctx.api.request(
			"wallet_getCurrentPermissions",
			null,
		);
		const hasReadPermissions =
			"result" in curr &&
			curr.result.find((r) => r.type === "account" && r.actions.read);
