...
export class UnisatBtcWalletHandler implements BtcWalletHandler {
	...
	async signMessage(
		...
		const type: SignMessageType =
			opts?.protocol === "bip322" ? "bip322-simple" : "ecdsa";
	...
	async signPsbt(psbtHex: string, opts: SignPsbtOpts): Promise<SignPsbtResult> {
		...
		const signedPsbtHex = await this._ctx.api.signPsbt(psbtHex, {
			toSignInputs,
		});
