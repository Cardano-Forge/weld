...
function newBtcWalletState(): PartialWithDiscriminant<
	BtcWalletProps,
	"isConnected"
> {
	return {
...
export type BtcWalletApi = {
	connect(
		key: string,
		config?: Partial<WalletConfig & ConnectBtcWalletCallbacks>,
	): void;
	connectAsync: (
		key: string,
		config?: Partial<WalletConfig>,
	) => Promise<ConnectedBtcWalletState>;
	disconnect(): void;
};

export type BtcWalletState<
	TKeys extends keyof BtcWalletProps = keyof BtcWalletProps,
> = PartialWithDiscriminant<BtcWalletProps, "isConnected", TKeys>;

export type ConnectedBtcWalletState = Extract<
	BtcWalletState,
	{ isConnected: true }
>;
export type DiconnectedBtcWalletState = Extract<
	BtcWalletState,
	{ isConnected: false }
>;
...
export type BtcWalletStore = Store<
	BtcWalletStoreState,
