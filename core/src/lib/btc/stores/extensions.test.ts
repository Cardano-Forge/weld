...
describe("updateExtensions", () => {
	it("should save valid extensions", () => {
		...
		const store = createBtcExtensionsStore({
			lifecycle,
		...
		expect(
			store.installedArr.find((ext) => ext.key === supported.key)?.info,
		).toBe(supported.info);
	...
	it("should always return the same objects when caching is enabled", () => {
		...
		store.updateExtensions();
		const fromArr1 = store.installedArr.find(
			(ext) => ext.info === supported.info,
		);
		...
		store.updateExtensions();
		const fromArr2 = store.installedArr.find(
			(ext) => ext.info === supported.info,
		);
