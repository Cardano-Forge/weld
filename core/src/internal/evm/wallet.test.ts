...
let api: { request: (...params: any[]) => unknown } | undefined;
			...
			switch (req.method) {
				...
				case "eth_getTransactionByHash":
					return {
						...
						blockHash:
							"0x95fc27f4ebc3463014f2f659cc3b81a660c77cf06169c33f144d7b10fb00cdb2",
...
describe("connectAsync", () => {
	...
	it("should switch network to provided chain id", async () => {
		const { wallet } = newTestStores();
		await wallet.connectAsync(walletKey);
		expect(evmRequestSpy).toHaveBeenCalledWith("wallet_switchEthereumChain", {
			chainId,
		});
	});
	...
	it("should fail if the extension is not installed", async () => {
		const { wallet } = newTestStores();
		await expect(() => wallet.connectAsync("phantom")).rejects.toThrow(
			WalletConnectionError,
		);
	});
...
describe("connect", () => {
	...
	it("should switch network to provided chain id", async () => {
		return new Promise<void>((done) => {
			...
			wallet.connect(walletKey, {
				onSuccess() {
					expect(evmRequestSpy).toHaveBeenCalledWith(
...
describe("getTokenBalance", () => {
	it("should fail when signer is not initialized", async () => {
		...
		await expect(() => wallet.getTokenBalance(ethTokenAddress)).rejects.toThrow(
			"Signer",
		);
	});
...
describe("send", () => {
	it("should fail when signer is not initialized", async () => {
		...
		wallet.setState({ signer: undefined });
		await expect(() =>
			wallet.send({ to: ethTokenAddress, amount: "2" }),
