...
type EventsWithoutParams<TProps> = Omit<
	Events<TProps>,
	keyof EventsWithParams<TProps>
>;
...
export class WalletStoreManager<
	TProps extends DefaultWalletStoreState = DefaultWalletStoreState,
> {
	...
	on<TEvent extends keyof Events<TProps>>(
		event: TEvent,
		handler: EventHandler<TProps, TEvent>,
	) {
	...
	async disconnect() {
		...
		if (this._ctx.configStore.enablePersistence) {
			this._ctx.configStore.storage.remove(
	...
	async connect(
		...
		try {
			...
			const connectTimeout =
				...
				this._ctx.configStore.wallet?.connectTimeout;
			if (connectTimeout) {
			...
			const connection = await this._ctx.createConnection(key, {
				signal,
				configOverrides,
			});
			...
			if (this._ctx.configStore.enablePersistence) {
				this._ctx.configStore.storage.set(
					STORAGE_KEYS[this._ctx.walletStorageKey],
					newState.key,
				);
			}
	...
	private async _runSubscriptions<
		TEvent extends keyof EventsWithParams<TProps>,
	>(event: TEvent, params: EventsWithParams<TProps>[TEvent]): Promise<void>;
	private async _runSubscriptions<
		TEvent extends keyof EventsWithoutParams<TProps>,
