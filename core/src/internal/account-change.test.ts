...
describe("handleAccountChangeErrors", () => {
	it("should reflect non-fct properties as is", () => {
		...
		const updateApi = vi.fn(() =>
			newApi({ balance: 100, name: "jack" }, { throws: false }),
		);
	...
	it("should try to update the api when account change error is thrown by an async fct", async () => {
		const api = newApi(
			{ balance: 100, name: "jack" },
			{ throws: "account-change-error" },
		);
		const updatedApi = newApi(
			{ balance: 500, name: "john" },
			{ throws: false },
		);
		...
		await expect(() => api.getBalanceAsync()).rejects.toThrow(
			TestAccountChangeError,
		);
		await expect(wrappedApi.getBalanceAsync()).resolves.toBe(
			updatedApi.info.balance,
		);
	});
	...
	it("should only retry once", async () => {
		...
		await expect(() => wrappedApi.getBalanceAsync()).rejects.toThrow(
			TestAccountChangeError,
		);
	});
